...
void cls_algsec::load_haar()
{
    try {
        if (model_file == "") {
            method = "none";
            MOTION_LOG(ERR, LOG_TYPE_ALL, NO_ERRNO, _("No secondary model specified."));
            return;
        }
        if (haar_cascade.load(model_file) == false) {
            /* Loading failed, reset method*/
            method = "none";
            MOTION_LOG(ERR, LOG_TYPE_ALL, NO_ERRNO, _("Failed loading model %s")
                ,model_file.c_str());
        }
    } catch ( cv::Exception& e ) {
        const char* err_msg = e.what();
...
void cls_algsec::params_log()
{
    ...
    if (method != "none") {
        for (indx=0;indx<params->params_cnt;indx++) {
            ...
            MOTION_SHT(INF, LOG_TYPE_ALL, NO_ERRNO, "%-25s %s"
...
void cls_algsec::handler()
{
    ...
    MOTION_LOG(INF, LOG_TYPE_ALL, NO_ERRNO,_("Secondary detection starting."));
    ...
    MOTION_LOG(INF, LOG_TYPE_ALL, NO_ERRNO,_("Secondary detection stopped."));
...
void cls_algsec::handler_startup()
{
    ...
    if (handler_running == false) {
        ...
        if (retcd != 0) {
            MOTION_LOG(WRN, LOG_TYPE_ALL, NO_ERRNO,_("Unable to start secondary detection"));
            handler_running = false;
            handler_stop = true;
        }
