...
void cls_config::edit_daemon(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","daemon",_("daemon"));
}
...
void cls_config::edit_conf_filename(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","log_file",_("log_file"));
}
...
void cls_config::edit_pid_file(std::string &parm, int pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","pid_file",_("pid_file"));
}
...
void cls_config::edit_log_level(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm_in < 1) || (parm_in > 9)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid log_level %d"),parm_in);
        } else {
            log_level = parm_in;
        }
    } else if (pact == PARM_ACT_GET) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","log_level",_("log_level"));
}
...
void cls_config::edit_log_fflevel(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if ((parm_in < 1) || (parm_in > 9)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid log_fflevel %d"),parm_in);
        } else {
            log_fflevel = parm_in;
        }
    } else if (pact == PARM_ACT_GET) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","log_fflevel",_("log_fflevel"));
}
...
void cls_config::edit_log_type(std::string &parm, enum PARM_ACT pact)
{
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm == "ALL") || (parm == "COR") ||
            ...
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid log_type %s"),parm.c_str());
        }
    } else if (pact == PARM_ACT_GET) {
        parm = log_type_str;
    } else if (pact == PARM_ACT_LIST) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","log_type",_("log_type"));
}
...
void cls_config::edit_native_language(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","native_language",_("native_language"));
}
...
void cls_config::edit_device_name(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","device_name",_("device_name"));
}
...
void cls_config::edit_device_id(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((this == app->cfg) || (this == app->conf_src)) {
            ...
            if (parm_in < 1) {
                MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid device_id %d"),parm_in);
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","device_id",_("device_id"));
}
...
void cls_config::edit_device_tmo(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if (parm_in < 1) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid device_tmo %d"),parm_in);
        } else {
            device_tmo = parm_in;
        }
    } else if (pact == PARM_ACT_GET) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","device_tmo",_("device_tmo"));
}
...
void cls_config::edit_pause(std::string &parm, enum PARM_ACT pact)
{
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm == "schedule") ||
            ...
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid pause %s"),parm.c_str());
        }
    } else if (pact == PARM_ACT_GET) {
        parm = pause;
    } else if (pact == PARM_ACT_LIST) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","pause",_("pause"));
}
...
void cls_config::edit_schedule_params(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","schedule_params",_("schedule_params"));
}
...
void cls_config::edit_cleandir_params(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","cleandir_params",_("cleandir_params"));
}
...
void cls_config::edit_config_dir(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","config_dir",_("config_dir"));
}
...
void cls_config::edit_target_dir(std::string &parm, enum PARM_ACT pact)
{
    if (pact == PARM_ACT_DFLT) {
        ...
        if (parm.find("%", 0) != std::string::npos) {
            ...
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO,"Removing trailing '/' from target_dir");
        } else {
            target_dir = parm;
        }
    } else if (pact == PARM_ACT_GET) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","target_dir",_("target_dir"));
}
...
void cls_config::edit_watchdog_tmo(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if (parm_in < 1) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid watchdog timeout %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","watchdog_tmo",_("watchdog_tmo"));
}
...
void cls_config::edit_watchdog_kill(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if (parm_in < 0) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid watchdog kill timeout %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","watchdog_kill",_("watchdog_kill"));
}
...
void cls_config::edit_v4l2_device(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","v4l2_device",_("v4l2_device"));
}
...
void cls_config::edit_v4l2_params(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","v4l2_params",_("v4l2_params"));
}
...
void cls_config::edit_netcam_url(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","netcam_url",_("netcam_url"));
}
...
void cls_config::edit_netcam_params(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","netcam_params",_("netcam_params"));
}
...
void cls_config::edit_netcam_high_url(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","netcam_high_url",_("netcam_high_url"));
}
...
void cls_config::edit_netcam_high_params(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","netcam_high_params",_("netcam_high_params"));
}
...
void cls_config::edit_netcam_userpass(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","netcam_userpass",_("netcam_userpass"));
}
...
void cls_config::edit_libcam_device(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","libcam_device",_("libcam_device"));
}
...
void cls_config::edit_libcam_params(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","libcam_params",_("libcam_params"));
}
...
void cls_config::edit_width(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm_in < 64) || (parm_in > 9999)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid width %d"),parm_in);
        } else if (parm_in % 8) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","width",_("width"));
}
...
void cls_config::edit_height(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if ((parm_in < 64) || (parm_in > 9999)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid height %d"),parm_in);
        } else if (parm_in % 8) {
            MOTION_LOG(CRT, TYPE_NETCAM, NO_ERRNO
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","height",_("height"));
}
...
void cls_config::edit_framerate(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm_in < 2) || (parm_in > 100)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid framerate %d"),parm_in);
        } else {
            framerate = parm_in;
        }
    } else if (pact == PARM_ACT_GET) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","framerate",_("framerate"));
}
...
void cls_config::edit_rotate(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm_in != 0) && (parm_in != 90) &&
            ...
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid rotate %d"),parm_in);
        } else {
            rotate = parm_in;
        }
    } else if (pact == PARM_ACT_GET) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","rotate",_("rotate"));
}
...
void cls_config::edit_flip_axis(std::string &parm, enum PARM_ACT pact)
{
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm == "none") || (parm == "vertical") || (parm == "horizontal")) {
            ...
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid flip_axis %s"), parm.c_str());
        }
    } else if (pact == PARM_ACT_GET) {
        parm = flip_axis;
    } else if (pact == PARM_ACT_LIST) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","flip_axis",_("flip_axis"));
}
...
void cls_config::edit_locate_motion_mode(std::string &parm, enum PARM_ACT pact)
{
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm == "off") || (parm == "on") || (parm == "preview")) {
          ...
          MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid locate_motion_mode %s"), parm.c_str());
        }
    } else if (pact == PARM_ACT_GET) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","locate_motion_mode",_("locate_motion_mode"));
}
...
void cls_config::edit_locate_motion_style(std::string &parm, enum PARM_ACT pact)
{
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm == "box") || (parm == "redbox") ||
            ...
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid locate_motion_style %s"), parm.c_str());
        }
    } else if (pact == PARM_ACT_GET) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","locate_motion_style",_("locate_motion_style"));
}
...
void cls_config::edit_text_left(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","text_left",_("text_left"));
}
...
void cls_config::edit_text_right(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","text_right",_("text_right"));
}
...
void cls_config::edit_text_changes(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","text_changes",_("text_changes"));
}
...
void cls_config::edit_text_scale(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm_in < 1) || (parm_in > 10)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid text_scale %d"),parm_in);
        } else {
            text_scale = parm_in;
        }
    } else if (pact == PARM_ACT_GET) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","text_scale",_("text_scale"));
}
...
void cls_config::edit_text_event(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","text_event",_("text_event"));
}
...
void cls_config::edit_emulate_motion(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","emulate_motion",_("emulate_motion"));
}
...
void cls_config::edit_threshold(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm_in < 1) || (parm_in > 2147483647)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid threshold %d"),parm_in);
        } else {
            threshold = parm_in;
        }
    } else if (pact == PARM_ACT_GET) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","threshold",_("threshold"));
}
...
void cls_config::edit_threshold_maximum(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if ((parm_in < 0) ) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid threshold_maximum %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","threshold_maximum",_("threshold_maximum"));
}
...
void cls_config::edit_threshold_sdevx(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if ((parm_in < 0) ) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid threshold_sdevx %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","threshold_sdevx",_("threshold_sdevx"));
}
...
void cls_config::edit_threshold_sdevy(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if ((parm_in < 0) ) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid threshold_sdevy %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","threshold_sdevy",_("threshold_sdevy"));
}
...
void cls_config::edit_threshold_sdevxy(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if ((parm_in < 0) ) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid threshold_sdevxy %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","threshold_sdevxy",_("threshold_sdevxy"));
}
...
void cls_config::edit_threshold_ratio(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm_in < 0) || (parm_in > 100) ) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid threshold_ratio %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","threshold_ratio",_("threshold_ratio"));
}
...
void cls_config::edit_threshold_ratio_change(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm_in < 0) || (parm_in > 255) ) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid threshold_ratio_change %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","threshold_ratio_change",_("threshold_ratio_change"));
}
...
void cls_config::edit_threshold_tune(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","threshold_tune",_("threshold_tune"));
}
...
void cls_config::edit_secondary_method(std::string &parm, enum PARM_ACT pact)
{
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm == "none") || (parm == "haar") ||
            ...
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid secondary_method %s"), parm.c_str());
        }
    } else if (pact == PARM_ACT_GET) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","secondary_method",_("secondary_method"));
}
...
void cls_config::edit_secondary_params(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","secondary_params",_("secondary_params"));
}
...
void cls_config::edit_noise_level(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm_in < 1) || (parm_in > 255)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid noise_level %d"),parm_in);
        } else {
            noise_level = parm_in;
        }
    } else if (pact == PARM_ACT_GET) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","noise_level",_("noise_level"));
}
...
void cls_config::edit_noise_tune(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","noise_tune",_("noise_tune"));
}
...
void cls_config::edit_despeckle_filter(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","despeckle_filter",_("despeckle_filter"));
}
...
void cls_config::edit_area_detect(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","area_detect",_("area_detect"));
}
...
void cls_config::edit_mask_file(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","mask_file",_("mask_file"));
}
...
void cls_config::edit_mask_privacy(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","mask_privacy",_("mask_privacy"));
}
...
void cls_config::edit_smart_mask_speed(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm_in < 0) || (parm_in > 10)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid smart_mask_speed %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","smart_mask_speed",_("smart_mask_speed"));
}
...
void cls_config::edit_lightswitch_percent(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if ((parm_in < 0) || (parm_in > 100)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid lightswitch_percent %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","lightswitch_percent",_("lightswitch_percent"));
}
...
void cls_config::edit_lightswitch_frames(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm_in < 1) || (parm_in > 1000)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid lightswitch_frames %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","lightswitch_frames",_("lightswitch_frames"));
}
...
void cls_config::edit_minimum_motion_frames(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm_in < 1) || (parm_in > 10000)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid minimum_motion_frames %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","minimum_motion_frames",_("minimum_motion_frames"));
}
...
void cls_config::edit_static_object_time(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if (parm_in < 1) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid static_object_time %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","static_object_time",_("static_object_time"));
}
...
void cls_config::edit_event_gap(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm_in < 0) || (parm_in > 2147483647)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid event_gap %d"),parm_in);
        } else {
            event_gap = parm_in;
        }
    } else if (pact == PARM_ACT_GET) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","event_gap",_("event_gap"));
}
...
void cls_config::edit_pre_capture(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm_in < 0) || (parm_in > 1000)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid pre_capture %d"),parm_in);
        } else {
            pre_capture = parm_in;
        }
    } else if (pact == PARM_ACT_GET) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","pre_capture",_("pre_capture"));
}
...
void cls_config::edit_post_capture(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if ((parm_in < 0) || (parm_in > 2147483647)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid post_capture %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","post_capture",_("post_capture"));
}
...
void cls_config::edit_on_event_start(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","on_event_start",_("on_event_start"));
}
...
void cls_config::edit_on_event_end(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","on_event_end",_("on_event_end"));
}
...
void cls_config::edit_on_picture_save(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","on_picture_save",_("on_picture_save"));
}
...
void cls_config::edit_on_area_detected(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","on_area_detected",_("on_area_detected"));
}
...
void cls_config::edit_on_motion_detected(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","on_motion_detected",_("on_motion_detected"));
}
...
void cls_config::edit_on_movie_start(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","on_movie_start",_("on_movie_start"));
}
...
void cls_config::edit_on_movie_end(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","on_movie_end",_("on_movie_end"));
}
...
void cls_config::edit_on_camera_lost(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","on_camera_lost",_("on_camera_lost"));
}
...
void cls_config::edit_on_camera_found(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","on_camera_found",_("on_camera_found"));
}
...
void cls_config::edit_on_secondary_detect(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","on_secondary_detect",_("on_secondary_detect"));
}
...
void cls_config::edit_on_action_user(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","on_action_user",_("on_action_user"));
}
...
void cls_config::edit_on_sound_alert(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","on_sound_alert",_("on_sound_alert"));
}
...
void cls_config::edit_on_lightswitch( std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","on_lightswitch","on_lightswitch");
}
...
void cls_config::edit_picture_output(std::string &parm, enum PARM_ACT pact)
{
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm == "on") || (parm == "off") || (parm == "center") ||
            ...
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid picture_output %s"), parm.c_str());
        }
    } else if (pact == PARM_ACT_GET) {
        parm = picture_output;
    } else if (pact == PARM_ACT_LIST) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","picture_output",_("picture_output"));
}
...
void cls_config::edit_picture_output_motion(std::string &parm, enum PARM_ACT pact)
{
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm == "on") || (parm == "off") || (parm == "roi"))  {
            ...
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid picture_output_motion %s"), parm.c_str());
        }
    } else if (pact == PARM_ACT_GET) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","picture_output_motion",_("picture_output_motion"));
}
...
void cls_config::edit_picture_type(std::string &parm, enum PARM_ACT pact)
{
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm == "jpg") || (parm == "webp") || (parm == "ppm"))  {
            ...
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid picture_type %s"), parm.c_str());
        }
    } else if (pact == PARM_ACT_GET) {
        parm = picture_type;
    } else if (pact == PARM_ACT_LIST) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","picture_type",_("picture_type"));
}
...
void cls_config::edit_picture_quality(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm_in < 1) || (parm_in > 100)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid picture_quality %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","picture_quality",_("picture_quality"));
}
...
void cls_config::edit_picture_exif(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","picture_exif",_("picture_exif"));
}
...
void cls_config::edit_picture_filename(std::string &parm, enum PARM_ACT pact)
{
    if (pact == PARM_ACT_DFLT) {
        ...
        if (parm == "") {
            ...
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO,"Removing leading '/' from filename");
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","picture_filename",_("picture_filename"));
}
...
void cls_config::edit_snapshot_interval(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if ((parm_in < 0) || (parm_in > 2147483647)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid snapshot_interval %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","snapshot_interval",_("snapshot_interval"));
}
...
void cls_config::edit_snapshot_filename(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","snapshot_filename",_("snapshot_filename"));
}
...
void cls_config::edit_movie_output(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","movie_output",_("movie_output"));
}
...
void cls_config::edit_movie_output_motion(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","movie_output_motion",_("movie_output_motion"));
}
...
void cls_config::edit_movie_max_time(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if ((parm_in < 0) || (parm_in > 2147483647)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid movie_max_time %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","movie_max_time",_("movie_max_time"));
}
...
void cls_config::edit_movie_bps(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm_in < 0) || (parm_in > 9999999)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid movie_bps %d"),parm_in);
        } else {
            movie_bps = parm_in;
        }
    } else if (pact == PARM_ACT_GET) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","movie_bps",_("movie_bps"));
}
...
void cls_config::edit_movie_quality(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if ((parm_in <= 0) || (parm_in > 100)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid movie_quality %d"),parm_in);
        } else if (parm_in > 90) {
            /*
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","movie_quality",_("movie_quality"));
}
...
void cls_config::edit_movie_container(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","movie_container",_("movie_container"));
}
...
void cls_config::edit_movie_passthrough(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","movie_passthrough",_("movie_passthrough"));
}
...
void cls_config::edit_movie_filename(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","movie_filename",_("movie_filename"));
}
...
void cls_config::edit_movie_retain(std::string &parm, enum PARM_ACT pact)
{
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm == "all") || (parm == "secondary") )  {
            ...
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid movie_retain %s"), parm.c_str());
        }
    } else if (pact == PARM_ACT_GET) {
        parm = movie_retain;
    } else if (pact == PARM_ACT_LIST) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","movie_retain",_("movie_retain"));
}
...
void cls_config::edit_movie_all_frames(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","movie_all_frames",_("movie_all_frames"));
}
...
void cls_config::edit_movie_extpipe_use(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","movie_extpipe_use",_("movie_extpipe_use"));
}
...
void cls_config::edit_movie_extpipe(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","movie_extpipe",_("movie_extpipe"));
}
...
void cls_config::edit_timelapse_interval(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if ((parm_in < 0) || (parm_in > 2147483647)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid timelapse_interval %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","timelapse_interval",_("timelapse_interval"));
}
...
void cls_config::edit_timelapse_mode(std::string &parm, enum PARM_ACT pact)
{
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm == "hourly") || (parm == "daily") ||
            ...
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid timelapse_mode %s"), parm.c_str());
        }
    } else if (pact == PARM_ACT_GET) {
        parm = timelapse_mode;
    } else if (pact == PARM_ACT_LIST) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","timelapse_mode",_("timelapse_mode"));
}
...
void cls_config::edit_timelapse_fps(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if ((parm_in < 2) || (parm_in > 100)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid timelapse_fps %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","timelapse_fps",_("timelapse_fps"));
}
...
void cls_config::edit_timelapse_container(std::string &parm, enum PARM_ACT pact)
{
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm == "mpg") || (parm == "mkv"))  {
            ...
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid timelapse_container %s"), parm.c_str());
        }
    } else if (pact == PARM_ACT_GET) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","timelapse_container",_("timelapse_container"));
}
...
void cls_config::edit_timelapse_filename(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","timelapse_filename",_("timelapse_filename"));
}
...
void cls_config::edit_video_pipe(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","video_pipe",_("video_pipe"));
}
...
void cls_config::edit_video_pipe_motion(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","video_pipe_motion",_("video_pipe_motion"));
}
...
void cls_config::edit_webcontrol_port(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm_in < 0) || (parm_in > 65535)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid webcontrol_port %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","webcontrol_port",_("webcontrol_port"));
}
...
void cls_config::edit_webcontrol_port2(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if ((parm_in < 0) || (parm_in > 65535)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid webcontrol_port2 %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","webcontrol_port2",_("webcontrol_port2"));
}
...
void cls_config::edit_webcontrol_base_path(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","webcontrol_base_path",_("webcontrol_base_path"));
}
...
void cls_config::edit_webcontrol_ipv6(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","webcontrol_ipv6",_("webcontrol_ipv6"));
}
...
void cls_config::edit_webcontrol_localhost(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","webcontrol_localhost",_("webcontrol_localhost"));
}
...
void cls_config::edit_webcontrol_parms(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm_in < 0) || (parm_in > 3)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid webcontrol_parms %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","webcontrol_parms",_("webcontrol_parms"));
}
...
void cls_config::edit_webcontrol_interface(std::string &parm, enum PARM_ACT pact)
{
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm == "default") || (parm == "user"))  {
            ...
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid webcontrol_interface %s"), parm.c_str());
        }
    } else if (pact == PARM_ACT_GET) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","webcontrol_interface",_("webcontrol_interface"));
}
...
void cls_config::edit_webcontrol_auth_method(std::string &parm, enum PARM_ACT pact)
{
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm == "none") || (parm == "basic") || (parm == "digest"))  {
            ...
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid webcontrol_auth_method %s"), parm.c_str());
        }
    } else if (pact == PARM_ACT_GET) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","webcontrol_auth_method",_("webcontrol_auth_method"));
}
...
void cls_config::edit_webcontrol_authentication(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","webcontrol_authentication",_("webcontrol_authentication"));
}
...
void cls_config::edit_webcontrol_tls(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","webcontrol_tls",_("webcontrol_tls"));
}
...
void cls_config::edit_webcontrol_cert(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","webcontrol_cert",_("webcontrol_cert"));
}
...
void cls_config::edit_webcontrol_key(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","webcontrol_key",_("webcontrol_key"));
}
...
void cls_config::edit_webcontrol_headers(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","webcontrol_headers",_("webcontrol_headers"));
}
...
void cls_config::edit_webcontrol_html(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","webcontrol_html",_("webcontrol_html"));
}
...
void cls_config::edit_webcontrol_actions(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","webcontrol_actions",_("webcontrol_actions"));
}
...
void cls_config::edit_webcontrol_lock_minutes(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if (parm_in < 0) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid webcontrol_lock_minutes %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","webcontrol_lock_minutes",_("webcontrol_lock_minutes"));
}
...
void cls_config::edit_webcontrol_lock_attempts(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if (parm_in < 0) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid webcontrol_lock_attempts %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","webcontrol_lock_attempts",_("webcontrol_lock_attempts"));
}
...
void cls_config::edit_webcontrol_lock_script(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","webcontrol_lock_script",_("webcontrol_lock_script"));
}
...
void cls_config::edit_stream_preview_scale(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if ((parm_in < 1) || (parm_in > 1000)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid stream_preview_scale %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","stream_preview_scale",_("stream_preview_scale"));
}
...
void cls_config::edit_stream_preview_newline(std::string &parm, enum PARM_ACT pact)
 {
     ...
     return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","stream_preview_newline",_("stream_preview_newline"));
 }
...
void cls_config::edit_stream_preview_params(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","stream_preview_params",_("stream_preview_params"));
}
...
void cls_config::edit_stream_preview_method(std::string &parm, enum PARM_ACT pact)
{
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm == "mjpg") || (parm == "static") || (parm == "combined"))  {
            ...
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid stream_preview_method %s"), parm.c_str());
        }
    } else if (pact == PARM_ACT_GET) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","stream_preview_method",_("stream_preview_method"));
}
...
void cls_config::edit_stream_preview_ptz(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","stream_preview_ptz",_("stream_preview_ptz"));
}
...
void cls_config::edit_stream_quality(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if ((parm_in < 1) || (parm_in > 100)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid stream_quality %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","stream_quality",_("stream_quality"));
}
...
void cls_config::edit_stream_grey(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","stream_grey",_("stream_grey"));
}
...
void cls_config::edit_stream_motion(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","stream_motion",_("stream_motion"));
}
...
void cls_config::edit_stream_maxrate(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if ((parm_in < 1) || (parm_in > 100)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid stream_maxrate %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","stream_maxrate",_("stream_maxrate"));
}
...
void cls_config::edit_stream_scan_time(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm_in < 1) || (parm_in > 600)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid stream_scan_time %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","stream_scan_time",_("stream_scan_time"));
}
...
void cls_config::edit_stream_scan_scale(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if ((parm_in < 1) || (parm_in > 1000)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid stream_scan_scale %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","stream_scan_scale",_("stream_scan_scale"));
}
...
void cls_config::edit_database_type(std::string &parm, enum PARM_ACT pact)
{
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm == "mariadb") || (parm == "") ||
            ...
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid database_type %s"), parm.c_str());
        }
    } else if (pact == PARM_ACT_GET) {
        parm = database_type;
    } else if (pact == PARM_ACT_LIST) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","database_type",_("database_type"));
}
...
void cls_config::edit_database_dbname(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","database_dbname",_("database_dbname"));
}
...
void cls_config::edit_database_host(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","database_host",_("database_host"));
}
...
void cls_config::edit_database_port(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if ((parm_in < 0) || (parm_in > 65535)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid database_port %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","database_port",_("database_port"));
}
...
void cls_config::edit_database_user(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","database_user",_("database_user"));
}
...
void cls_config::edit_database_password(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","database_password",_("database_password"));
}
...
void cls_config::edit_database_busy_timeout(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm_in < 0) || (parm_in > 10000)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid database_busy_timeout %d"),parm_in);
        } else {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","database_busy_timeout",_("database_busy_timeout"));
}
...
void cls_config::edit_sql_event_start(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","sql_event_start",_("sql_event_start"));
}
...
void cls_config::edit_sql_event_end(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","sql_event_end",_("sql_event_end"));
}
...
void cls_config::edit_sql_movie_start(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","sql_movie_start",_("sql_movie_start"));
}
...
void cls_config::edit_sql_movie_end(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","sql_movie_end",_("sql_movie_end"));
}
...
void cls_config::edit_sql_pic_save(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","sql_pic_save",_("sql_pic_save"));
}
...
void cls_config::edit_ptz_auto_track(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","ptz_auto_track",_("ptz_auto_track"));
}
...
void cls_config::edit_ptz_wait(std::string &parm, enum PARM_ACT pact)
{
    ...
    if (pact == PARM_ACT_DFLT) {
        ...
        parm_in = atoi(parm.c_str());
        if ((parm_in < 0) || (parm_in > 2147483647)) {
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid ptz_wait %d"),parm_in);
        } else {
            ptz_wait = parm_in;
        }
    } else if (pact == PARM_ACT_GET) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","ptz_wait",_("ptz_wait"));
}
...
void cls_config::edit_ptz_move_track(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","ptz_move_track",_("ptz_move_track"));
}
...
void cls_config::edit_ptz_pan_left(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","ptz_pan_left",_("ptz_pan_left"));
}
...
void cls_config::edit_ptz_pan_right(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","ptz_pan_right",_("ptz_pan_right"));
}
...
void cls_config::edit_ptz_tilt_up(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","ptz_tilt_up",_("ptz_tilt_up"));
}
...
void cls_config::edit_ptz_tilt_down(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","ptz_tilt_down",_("ptz_tilt_down"));
}
...
void cls_config::edit_ptz_zoom_in(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","ptz_zoom_in",_("ptz_zoom_in"));
}
...
void cls_config::edit_ptz_zoom_out(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","ptz_zoom_out",_("ptz_zoom_out"));
}
...
void cls_config::edit_snd_device(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","snd_device",_("snd_device"));
}
...
void cls_config::edit_snd_params(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","snd_params",_("snd_params"));
}
...
void cls_config::edit_snd_alerts(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","snd_alerts",_("snd_alerts"));
}
...
void cls_config::edit_snd_window(std::string &parm, enum PARM_ACT pact)
{
    if (pact == PARM_ACT_DFLT) {
        ...
        if ((parm == "none") || (parm == "hamming") || (parm == "hann")) {
            ...
            MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Invalid snd_window %s"), parm.c_str());
        }
    } else if (pact == PARM_ACT_GET) {
        parm = snd_window;
    } else if (pact == PARM_ACT_LIST) {
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","snd_window",_("snd_window"));
}
...
void cls_config::edit_snd_show(std::string &parm, enum PARM_ACT pact)
{
    ...
    return;
    MOTION_LOG(DBG, LOG_TYPE_ALL, NO_ERRNO,"%s:%s","snd_show",_("snd_show"));
}
...
int cls_config::edit_set_depr(std::string &parm_nm, std::string &parm_val)
{
    ...
    while (config_parms_depr[indx].parm_name != "") {
        if (parm_nm ==  config_parms_depr[indx].parm_name) {
            MOTION_LOG(ALR, LOG_TYPE_ALL, NO_ERRNO, "%s after version %s"
...
void cls_config::edit_set(std::string parm_nm, std::string parm_val)
{
    ...
    MOTION_LOG(ALR, LOG_TYPE_ALL, NO_ERRNO, _("Unknown config option \"%s\""), parm_nm.c_str());
}
...
void cls_config::camera_add(std::string fname, bool srcdir)
{
    ...
    if (fname == "") {
        ...
        MOTION_LOG(ALR, LOG_TYPE_ALL, SHOW_ERRNO
...
void cls_config::parms_log_parm(std::string parm_nm, std::string parm_vl)
{
    ...
    {
        MOTION_SHT(INF, LOG_TYPE_ALL, NO_ERRNO
...
void cls_config::init()
{
    ...
    if (filename == "") {
        if (getcwd(path, sizeof(path)) == NULL) {
            MOTION_LOG(ERR, LOG_TYPE_ALL, SHOW_ERRNO, _("Error getcwd"));
            exit(-1);
        }
    ...
    if (filename == "") {
        ...
        if (filename != "") {
            MOTION_LOG(WRN, LOG_TYPE_ALL, SHOW_ERRNO
