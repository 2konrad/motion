...
void cls_sound::edit_alerts()
{
    ...
    if (validids == false) {
        MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, "Sound alert ids must be unique.");
        MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, "Creating new sound alert ids.");
        indx = 0;
        for (it_a0=snd_info->alerts.begin(); it_a0!=snd_info->alerts.end(); it_a0++) {
            it_a0->alert_id = indx;
            indx++;
    ...
    for (it_a0=snd_info->alerts.begin(); it_a0!=snd_info->alerts.end(); it_a0++) {
        ...
        MOTION_LOG(INF, LOG_TYPE_ALL, NO_ERRNO, "Sound Alert Parameters:");
        MOTION_LOG(INF, LOG_TYPE_ALL, NO_ERRNO, "  alert_id:            %d",it_a0->alert_id);
        MOTION_LOG(INF, LOG_TYPE_ALL, NO_ERRNO, "  alert_nm             %s",it_a0->alert_nm.c_str());
        MOTION_LOG(INF, LOG_TYPE_ALL, NO_ERRNO, "  freq_low:            %.4f",it_a0->freq_low);
        MOTION_LOG(INF, LOG_TYPE_ALL, NO_ERRNO, "  freq_high:           %.4f",it_a0->freq_high);
        MOTION_LOG(INF, LOG_TYPE_ALL, NO_ERRNO, "  volume_count:        %d",it_a0->volume_count);
        MOTION_LOG(INF, LOG_TYPE_ALL, NO_ERRNO, "  volume_level:        %d",it_a0->volume_level);
        MOTION_LOG(INF, LOG_TYPE_ALL, NO_ERRNO, "  trigger_threshold:   %d",it_a0->trigger_threshold);
        MOTION_LOG(INF, LOG_TYPE_ALL, NO_ERRNO, "  trigger_duration:    %d",it_a0->trigger_duration);
    }
...
void cls_sound::load_params()
{
    ...
    for (indx=0;indx<snd_info->params->params_cnt;indx++) {
        ...
        MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, "%s : %s"
...
void cls_sound::alsa_list_subdev()
{
    ...
    MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("  Subdevices: %i/%i")
...
void cls_sound::alsa_list_card()
{
    ...
    if (retcd < 0) {
        MOTION_LOG(ERR, LOG_TYPE_ALL, NO_ERRNO, _("snd_ctl_pcm_next_device"));
        return;
    }
...
void cls_sound::alsa_list()
{
    ...
    MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Devices"));
    ...
    while (alsa->card_id >= 0) {
        ...
        if (retcd == 0) {
            ...
            MOTION_LOG(ERR, LOG_TYPE_ALL, NO_ERRNO, _("control open (%i): %s")
                , alsa->card_id, snd_strerror(retcd));
        }
...
void cls_sound::alsa_start()
{
    ...
    MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Actual rate %hu"), actl_rate);
    MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Actual frames per %lu"), frames_per);
    if (actl_sndfmt <= 5) {
        MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Sound format 16"));
    } else if (actl_sndfmt <= 9 ) {
        MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Sound format 24"));
    } else {
        MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Sound format 32"));
    }
    ...
    MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, "Started.");
...
void cls_sound::cleanup()
{
    ...
    MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, "Stopped.");
...
void cls_sound::init()
{
    ...
    MOTION_LOG(INF, LOG_TYPE_ALL, NO_ERRNO,_("Initialize sound frequency"));
    ...
    if ((snd_info->source != "alsa") &&
        ...
        MOTION_LOG(ERR, LOG_TYPE_ALL, NO_ERRNO,_("Invalid sound source."));
        device_status = STATUS_CLOSED;
        return;
    }
    ...
    MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Detecting"));
...
void cls_sound::handler()
{
    ...
    MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Sound exiting"));
...
void cls_sound::handler_startup()
{
    ...
    pthread_attr_t thread_attr;
        ...
        MOTION_LOG(NTC, LOG_TYPE_ALL, NO_ERRNO, _("Required packages not installed"));
        device_status = STATUS_CLOSED;
        return;
    #endif
    ...
    if (handler_running == false) {
        ...
        if (retcd != 0) {
            MOTION_LOG(WRN, LOG_TYPE_ALL, NO_ERRNO,_("Unable to start sound frequency detection loop."));
            handler_running = false;
            handler_stop = true;
        }
